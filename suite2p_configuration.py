...
from suite2p.io import tiff, h5, save
...
def default_ops():
    ops = {
        ...
        'tau':  7., # this is the main parameter for deconvolution
        ...
        'diameter': 7, # if not sparse_mode, use diameter for filtering and extracting
...
def run_s2p(ops={},db={}):
    ...
    if not files_found_flag:
        ...
        if ops['input_format'] == 'h5':
            ...
            elif ops['input_format'] == 'EnricoCustom':
                print('time %4.2f sec. Using EnricoCustom')
    ...
    while ipl<len(ops1):
        print('>>>>>>>>>>>>>>>>>>>>> PLANE %d <<<<<<<<<<<<<<<<<<<<<<'%ipl)
        t1 = time.time()
        if ops['filtereTheDataBeforeRegistration']:
            print('----------- Filtering registered binary', ipl)
            filter_binary(ops1[ipl], dtype=np.uint16)
        if not flag_binreg:
            ######### REGISTRATION #########
            t11=time.time()
            print('----------- REGISTRATION')
        ...
        if ops['filtereTheDataAfterRegistration']:
            print('----------- Filtering registered binary', ipl)
            filter_binary(ops1[ipl], dtype=np.int16)
        ...
        if roidetect:
            ...
            if ops['avoidSpikeDeconvolution']:
                print("skipping spike deconvolutuion")
...
def tosuite2p(ops):
        ...
        input_path = ops['data_path'][0]  +  '\Image_001_001.raw'
        fpr = np.memmap(input_path, dtype=np.uint16, mode='r')
        frame_size =  ops['heigth'] * ops['width'] 
        
        if  ops['max_frames_to_load'] > 0:
            total_number_of_frames = ops['max_frames_to_load'] #/ frame_size
        else:
            total_number_of_frames = fpr.shape[0] / frame_size
        number_of_frames_per_plane = int( total_number_of_frames / ops['nplanes'] ) 
        actual_number_of_frames = number_of_frames_per_plane  *  ops['nplanes']
        start = ops['nframes_to_skip']  *  frame_size
            
        end =  actual_number_of_frames * frame_size   + start
        print(start, end, actual_number_of_frames, fpr.shape)
        rawdata = fpr[start : end].reshape(-1, ops['heigth'], ops['width'] )
        ...
        while True:
            ...
            im = rawdata[irange[0]:irange[-1]+1, :, :]
            nframes = im.shape[0]    #mod by Chiarello
            print("im.shape", im.shape)
            for j in range(0,nplanes):        #mod by Chiarello
                if i0==0:
                ...
                if ops['filtereTheData'] > 0:
                    ...
                    weights = ops['convKernel']
                    print("before filtering")
  
                    im2write = scipy.ndimage.filters.convolve1d(im2write, weights, axis=0)
...
def filter_binary(ops, dtype):
   
    input_path = ops['reg_file']
    raw = np.fromfile(input_path, dtype=np.uint32, count=-1).reshape([-1, ops['heigth'], ops['width'] ])
    
    weights = ops['convKernel']
    
    raw = scipy.ndimage.filters.convolve1d(raw, weights, axis=0)
    
    reg_file = open(ops['reg_file'], 'wb')
    reg_file.write(bytearray(raw))  
    reg_file.close()
...
this_ops = {
        # file paths
        'data_path' : [ r'PATH\TO\FILE.raw' ],
        # main settings
        'nplanes' : 30, # each tiff has these many planes in sequence
        'tau':  7, # this is the main parameter for deconvolution
        'fs': 1,  # sampling rate (PER PLANE e.g. for 12 plane recordings it will be around 2.5)
        ...
        'width': 1024,
        'heigth' : 512,
        'nframes_to_skip' : 30 * 300,
        'max_frames_to_load' : -1,  #  -1 load all
        'filtereTheData': 0,   
